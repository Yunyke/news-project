...
public class CartController {
	...
	public ResponseEntity<String> addToCart(@PathVariable Integer userId, @RequestBody Map<String, Long> payload) {
	...
	public ResponseEntity<Void> removeFromCart(@PathVariable Integer userId, @PathVariable Long newsId) {
		cartService.removeNewsFromCart(userId, newsId);
		return ResponseEntity.noContent().build();
	}

	@PostMapping("/dailynews/users/{userId}/favorites/{newsId}")
	public ResponseEntity<FavoriteToggleResponse> toggleFavorite(@PathVariable Integer userId,
