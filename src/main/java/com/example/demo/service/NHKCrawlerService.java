...
public class NHKCrawlerService {
	...
	private static final Pattern DETAIL_PROP_MORE = Pattern.compile("more:\\s*'([^']+)'", Pattern.DOTALL);
	...
	public void enrich(NHKNews news) {
		try {
			...
			Elements bodyParagraphs = doc.select("div.content--detail-body div.body-text > p");

			if (bodyParagraphs.isEmpty()) {
				Element detailScript = doc.selectFirst("script:containsData(__DetailProp__)");
