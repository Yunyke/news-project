...
public class UserMapper {
	...
	public void init() {
		...
		modelMapper.typeMap(User.class, UserDto.class).addMapping(User::getId, UserDto::setUserId);

		// 反向對應：UserDto.userId -> User.id (已有的)
		modelMapper.typeMap(UserDto.class, User.class).addMapping(UserDto::getUserId, User::setId);

		// User.id -> UserCert.userId
		modelMapper.typeMap(User.class, UserCert.class).addMapping(User::getId, UserCert::setUserId);

		// UserCert.userId -> User.id
		modelMapper.typeMap(UserCert.class, User.class).addMapping(UserCert::getUserId, User::setId);
	}
